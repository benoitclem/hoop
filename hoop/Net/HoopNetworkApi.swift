...
extension HoopNetworkApi {
    ...
    func getHoopsInfos(by location: CLLocation) -> Future<JSON>? {
        guard let token = self.appToken else {
            return nil
        }
        
        return self.request(with: "getLovestopInfoByLatLong",and: ["token":token,"lat":String(location.coordinate.latitude),"long":String(location.coordinate.longitude), "margin":"0.02"])
    }
    
    func signUp(with facebookData: [String:String]) -> Future<JSON>? {
        //print("registering new user")
        // Serialize the facebook data
        //jsonData?["data"]["token"].rawString(),jsonData?["data"]["sharing_code"].rawString()
        return self.post(with: "signUpClient", and: facebookData, andProgress: nil)
    }
    ...
    func getLoveStopInIds(by location: CLLocationCoordinate2D) -> Future<[Int]>? {
        ...
        return promise.then(on: DispatchQueue.main) { json -> Future<[Int]> in
    ...
    func getLoveStopIn(by location: CLLocationCoordinate2D) -> Future<[hoop]>? {
        guard let token = self.appToken else {
            return nil
        }
        
        self.getLoveStopIn(by: location).then(on: DispatchQueue.main) { hoopIndexArray -> Future<[hoop]>? in
            
        }
        
        self.request(with: "getLovestopInfoById",and: ["token":token,"id":String(location.latitude),"long":String(location.longitude)])
    }
}
